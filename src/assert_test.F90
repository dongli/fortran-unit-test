program test_assert
  ...
  type(test_suite_type) :: test_suite_shape_mismatch
  ...
  ! in test_suite_shape_mismatch all asserts should be false
  call test_suite_init('Shape mismatch', test_suite_shape_mismatch)

  ! test assert_approximate routines
  call test_case_create('approximate', test_suite_shape_mismatch)

  ! -> real (real4, real8)
  ! vector
  call assert_approximate([1.0, 1.0], [1.0], eps=0.01, suite=test_suite_shape_mismatch)
  call assert_approximate([1.0D0, 1.0D0], [1.0D0], eps=0.01D0, suite=test_suite_shape_mismatch)

  ! array
  call assert_approximate(reshape([1.0, 1.0, 1.0, 1.0], [2, 2]), reshape([1.0, 1.0], [2, 1]), eps=0.01, suite=test_suite_shape_mismatch)
  call assert_approximate(reshape([1.0D0, 1.0D0, 1.0D0, 1.0D0], [2, 2]), reshape([1.0D0, 1.0D0], [2, 1]), eps=0.01D0, suite=test_suite_shape_mismatch)

  ! test assert_equal routines
  call test_case_create('equal', test_suite_shape_mismatch)

  ! string
  ! vector
  call assert_equal(['abc', 'abc'], ['abc'], __FILE__, __LINE__, test_suite_shape_mismatch)

  ! array
  call assert_equal(reshape(['abc', 'abc', 'abc', 'abc'], [2, 2]), reshape(['abc', 'abc'], [2, 1]), __FILE__, __LINE__, test_suite_shape_mismatch)

  ! -> integer (int8->kind=1, int16->kind=2, int32->kind=4, int64->kind=8)
  ! vector
  call assert_equal([int(1, 1), int(1, 1), int(1, 1), int(1, 1)], [int(1, 1), int(1, 1), int(1, 1)], __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_equal([int(1, 2), int(1, 2), int(1, 2), int(1, 2)], [int(1, 2), int(1, 2), int(1, 2)], __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_equal([int(1, 4), int(1, 4), int(1, 4), int(1, 4)], [int(1, 4), int(1, 4), int(1, 4)], __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_equal([int(1, 8), int(1, 8), int(1, 8), int(1, 8)], [int(1, 8), int(1, 8), int(1, 8)], __FILE__, __LINE__, test_suite_shape_mismatch)

  ! array
  call assert_equal(reshape([int(1, 1), int(1, 1), int(1, 1), int(1, 1)], [2, 2]), &
      & reshape([int(1, 1), int(1, 1)], [2, 1]), __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_equal(reshape([int(1, 2), int(1, 2), int(1, 2), int(1, 2)], [2, 2]), &
      & reshape([int(1, 2), int(1, 2)], [2, 1]), __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_equal(reshape([int(1, 4), int(1, 4), int(1, 4), int(1, 4)], [2, 2]), &
      & reshape([int(1, 4), int(1, 4)], [2, 1]), __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_equal(reshape([int(1, 8), int(1, 8), int(1, 8), int(1, 8)], [2, 2]), &
      & reshape([int(1, 8), int(1, 8)], [2, 1]), __FILE__, __LINE__, test_suite_shape_mismatch)

  ! -> real (real4, real8)
  ! vector
  call assert_equal([1.0, 1.0], [1.0], __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_equal([1.0D0, 1.0D0], [1.0D0], __FILE__, __LINE__, test_suite_shape_mismatch)

  ! array
  call assert_equal(reshape([1.0D0, 1.0D0, 1.0D0, 1.0D0], [2, 2]), reshape([1.0D0, 1.0D0], [2, 1]), __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_equal(reshape([1.0, 1.0, 1.0, 1.0], [2, 2]), reshape([1.0, 1.0], [2, 1]), __FILE__, __LINE__, test_suite_shape_mismatch)

  ! test assert_great_than routines
  call test_case_create('great_than', test_suite_shape_mismatch)

  ! -> integer (int8->kind=1, int16->kind=2, int32->kind=4, int64->kind=8)
  ! vector
  call assert_great_than([int(1, 1), int(1, 1), int(1, 1), int(1, 1)], [int(1, 1), int(1, 1), int(1, 1)], __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_great_than([int(1, 2), int(1, 2), int(1, 2), int(1, 2)], [int(1, 2), int(1, 2), int(1, 2)], __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_great_than([int(1, 4), int(1, 4), int(1, 4), int(1, 4)], [int(1, 4), int(1, 4), int(1, 4)], __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_great_than([int(1, 8), int(1, 8), int(1, 8), int(1, 8)], [int(1, 8), int(1, 8), int(1, 8)], __FILE__, __LINE__, test_suite_shape_mismatch)
  ...
  ! -> real (real4, real8)
  ! vector
  call assert_great_than([1.0, 1.0], [1.0], __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_great_than([1.0D0, 1.0D0], [1.0D0], __FILE__, __LINE__, test_suite_shape_mismatch)

  ! array
  call assert_great_than(reshape([1.0, 1.0, 1.0, 1.0], [2, 2]), reshape([1.0, 1.0], [2, 1]), __FILE__, __LINE__, test_suite_shape_mismatch)
  call assert_great_than(reshape([1.0D0, 1.0D0, 1.0D0, 1.0D0], [2, 2]), reshape([1.0D0, 1.0D0], [2, 1]), __FILE__, __LINE__, test_suite_shape_mismatch)

  call test_suite_report(test_suite_shape_mismatch)
  call test_suite_final(test_suite_shape_mismatch)
